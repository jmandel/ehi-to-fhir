 ...
 * answer-key GAP ledger (compare/LEDGER.json after `classify.ts --out=out-answerkey`).
...
export function verdict(g:any):[string,string]{
  ...
  // Specimen.type: the real Epic .300 code (100230 Serum) IS already emitted via the answer-key; the ONLY
  ...
  if(rt==="Specimen"&&p.startsWith("type")) return["FLOOR","target SNOMED is a codeless {system}-only stub on serum; real .300 code already emitted via answer-key (round-7 verified)"];
