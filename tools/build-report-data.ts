 ...
 *   report/viewer/data.json       canonical view (crosswalk / terminology bridge ON)
 ...
 * canonical (crosswalk) state. Run the generators first:
 *   bun build.ts --apply-crosswalk --embed-attachments     # writes out/ (lean) AND out-crosswalk/
 * then:
...
console.log("→ lean view (out/, no crosswalk)");
...
console.log("→ canonical view (out-crosswalk/, crosswalk + attachments)");
run(["bun", "compare/classify.ts", "--out=out-crosswalk"], { EXCLUDE_SMARTDATA: "1" });
run(["bun", "tools/build-viewer.ts"]); // defaults: out-crosswalk + compare/LEDGER.json -> data.json
...
console.log("compare/LEDGER.json left in canonical (crosswalk) state. Next: bun report/build.ts");
