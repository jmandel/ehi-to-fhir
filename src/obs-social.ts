 ...
 */
import { q, q1, dateRealToISO as dateRealIso } from "../lib/db";
import { id, patientRef } from "../lib/ids";
import { emit, clean } from "../lib/gen";
import { cc, category } from "../lib/cc";
import { localMidnightToUtcInstant } from "../lib/time";

const CATEGORY_SOCIAL_HISTORY = category(
  cc("http://terminology.hl7.org/CodeSystem/observation-category", "social-history", "Social History")
);

/**
 * "YYYY-MM-DD" calendar day -> UTC instant for the configured-org local midnight.
 * The social-history `issued` instants in the target are the snapshot/review calendar
 * date at local midnight expressed in UTC: a summer date (CDT, UTC-5) renders
 * ...T05:00:00Z, a winter date (CST, UTC-6) renders ...T06:00:00Z.
 * (Verified: smoking 2024-07-02 -> 05:00Z, alcohol/drug 2025-12-04 -> 06:00Z.)
 */
const centralMidnightToUtc = (isoDate: string): string | undefined =>
  localMidnightToUtcInstant(isoDate);
